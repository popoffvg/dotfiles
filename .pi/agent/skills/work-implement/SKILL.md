...
3. **Run static analysis** on changed files before testing. Use the appropriate language-specific linter/checker for the file type. Fix all errors before proceeding.
4. Run relevant tests for that TODO
5. If tests fail, fix and **re-test** until passing
6. Verify the TODO is fully satisfied (code + tests)
7. If test coverage is narrow/insufficient for the change, note that in `_notes/worklog.md` and tell the user manual verification is required
8. **Stage changes** with `git add` for the files you changed
9. **Launch subagent review.** Call the `subagent` tool:
   ```
   agent: "work-reviewer"
...
10. If `approveCommits` is enabled in settings: **Ask the user for approval before committing.** Show: TODO text, changed files, test results, and **reviewer verdict**. Wait for explicit "yes"/"ok"/"approve". If rejected — fix and ask again (re-run reviewer after fixes).
11. **Commit the changes** with a meaningful message referencing the TODO (commit only after steps 1–10 are complete)
12. Check off the TODO: `- [ ]` → `- [x]`
13. Log what was done to `_notes/worklog.md`
14. **Call `work_compact`** with a brief summary of what was completed — this frees context space and re-injects the plan so you stay oriented
15. Continue to the next TODO
...
- ❌ Forbidden: adding new TODOs, removing TODOs, rewriting TODO text, editing descriptions, modifying Design Decisions
