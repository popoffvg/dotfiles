...
impl Trace {
    /// `LINE_COMMENT_LOG=off` disables it; any other value is the path to write.
    pub fn open() -> Trace {
        let path = match std::env::var("LINE_COMMENT_LOG") {
            Ok(value) if value == "off" => return Trace { file: None, pid: 0 },
            Ok(value) => PathBuf::from(value),
            Err(_) => std::env::temp_dir().join("line-comment-lsp.log"),
        };
