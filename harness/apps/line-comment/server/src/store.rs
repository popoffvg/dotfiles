...
pub struct Comment {
    /// 1-based line in the file, matching the export format. The line the anchor hash is
    /// taken from, and the line a comment is addressed by.
    pub line: usize,
    /// Last line a comment written over a selection covers. Absent for a comment on
    /// `line` alone, which keeps a store written before selections loadable and keeps the
    /// single-line comment — still the common one — the shape it always had.
...
impl Comment {
    /// The last line this comment covers — `line` itself when it covers only that one.
    pub fn last_line(&self) -> usize {
        self.end_line.unwrap_or(self.line).max(self.line)
    }

    /// Set the lines covered, dropping the end when it adds nothing to the start.
    pub fn cover(&mut self, line: usize, end_line: usize) {
        self.line = line.max(1);
        self.end_line = Some(end_line.max(self.line)).filter(|end| *end > self.line);
    }

    /// Whether the comment covers this 1-based line.
    pub fn covers(&self, line: usize) -> bool {
        self.line <= line && line <= self.last_line()
    }
}
