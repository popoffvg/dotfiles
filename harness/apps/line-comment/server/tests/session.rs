...
use line_comment::store::{Author, Comment, Store};
use line_comment::wire::{
    Change, Position, Range, COMMAND_ADD, COMMAND_COPY, COMMAND_DELETE, COMMAND_LIST,
    COMMAND_RESET, SEVERITY_HINT,
};
use line_comment::{anchor, Effect, Session};
use serde_json::json;
...
fn root() -> PathBuf {
    ...
    let path =
        std::env::temp_dir().join(format!("line-comment-test-{}-{unique}", std::process::id()));
...
fn open(text: &str) -> (Session, PathBuf, String) {
    let root = root();
    let uri = line_comment::path_to_uri(&root.join("docs").join("spec.md"));
...
fn add(session: &mut Session, uri: &str, line: u32, text: &str) {
    ...
    // A commented line is handed back its own text; the operator types over it.
    let header = contents.lines().next().unwrap_or_default();
    write_input(&path, &format!("{header}\n\n{text}\n"));
    session.drain_input();
}
...
#[test]
fn editing_a_commented_line_hands_back_the_text_it_holds() {
    let (mut session, _root, uri) = open("# Title\n\n## Design\n");
    add(&mut session, &uri, 2, "first line\n\nsecond line");
...
/// Run the add command over a selection and fill the input file it mints. Lines are
/// 0-based, as on the LSP wire, and the selection ends at the end of `end_line`.
fn add_over(session: &mut Session, uri: &str, line: u32, end_line: u32, text: &str) {
    let actions = session.code_actions(
        uri,
        Range {
            start: at(line),
            end: Position {
    ...
    let add = actions
        .iter()
        .find(|action| action.command.command == COMMAND_ADD)
        .expect("a selection offers add comment");
    assert_eq!(
        add.command.arguments,
        vec![json!(uri), json!(line + 1), json!(end_line + 1)]
    );
    let effects = session.execute_command(COMMAND_ADD, &add.command.arguments);
    let (path, contents) = effects
        .iter()
        .find_map(|effect| match effect {
            Effect::OpenInput { path, contents } => Some((path.clone(), contents.clone())),
            _ => None,
...
#[test]
fn a_selection_comments_every_line_it_covers() {
    let (mut session, _root, uri) = open("a\nb\nc\nd\n");
    add_over(&mut session, &uri, 1, 2, "about b and c");

    let stored = comment(&session, 0);
    assert_eq!(stored.line, 2);
    assert_eq!(stored.last_line(), 3);
    assert_eq!(stored.text, "about b and c");
    assert_eq!(stored.hash, anchor::line_hash("b"));

    // The export names the span, and the diagnostic underlines all of it.
    assert!(
        session
            .export_text()
            .contains("**docs/spec.md** lines 2-3 (RIGHT)\n"),
        "{}",
        session.export_text()
    );
    let payload = session
    ...
    assert_eq!(payload.diagnostics[0].range.start, at(1));
    assert_eq!(
        payload.diagnostics[0].range.end,
        Position {
            line: 2,
            character: 1
        }
...
#[test]
fn a_selection_of_whole_lines_stops_at_the_last_one() {
    let (session, _root, uri) = open("a\nb\nc\n");
    // Selecting the first two lines whole: the editor reports the end as line 3, column 0.
    let actions = session.code_actions(
        &uri,
        Range {
            start: at(0),
    ...
    assert_eq!(actions[0].title, "add comment on lines 1-2");
    assert_eq!(
        actions[0].command.arguments,
        vec![json!(&uri), json!(1), json!(2)]
    );
}

#[test]
fn every_line_of_a_span_offers_edit_and_delete() {
    let (mut session, _root, uri) = open("a\nb\nc\nd\n");
    add_over(&mut session, &uri, 1, 2, "about b and c");

    // The cursor stands on the last line of the span, not on the line it is anchored to.
    let actions = session.code_actions(
        &uri,
        Range {
            start: at(2),
    ...
    assert_eq!(
        ...
        vec![
            "add comment",
            "edit comment: about b and c",
            "delete comment: about b and c",
            "list comments",
            "copy comments",
            "reset comments"
        ]
    ...
    // Editing keeps the lines the comment covers, wherever the cursor stood.
    let edit = actions
        .iter()
        .find(|action| action.title.starts_with("edit comment"))
        .expect("the span offers edit from its last line");
    let effects = session.execute_command(COMMAND_ADD, &edit.command.arguments);
...
#[test]
fn a_line_typed_inside_a_span_grows_it() {
    let (mut session, _root, uri) = open("a\nb\nc\nd\n");
    add_over(&mut session, &uri, 1, 2, "about b and c");
    ...
    let stored = comment(&session, 0);
    assert_eq!(stored.line, 2);
    assert_eq!(stored.last_line(), 4);
}

#[test]
fn a_span_moves_whole_when_its_first_line_moves() {
    let (mut session, _root, uri) = open("a\nb\nc\nd\n");
    add_over(&mut session, &uri, 1, 2, "about b and c");

    // The file is rewritten from outside, pushing the anchored line down two.
    session.did_open(&uri, "x\ny\na\nb\nc\nd\n".to_string());

    let stored = comment(&session, 0);
    assert_eq!(stored.line, 4);
    assert_eq!(stored.last_line(), 5);
    assert!(!stored.orphaned);
}
...
fn a_commented_line_offers_edit_and_delete_instead_of_add() {
    ...
    assert_eq!(
        commented[0].command.arguments,
        vec![json!(uri), json!(1), json!(1)]
    );
...
fn a_clean_line_offers_add() {
    ...
    assert_eq!(
        clean[0].command.arguments,
        vec![json!(uri), json!(2), json!(2)]
    );
}
...
fn a_leftover_input_file_is_taken_at_startup() {
    ...
    assert_eq!(stored.hash, line_comment::anchor::line_hash("## Design"));
    assert!(session.input_paths().is_empty());
}
...
fn the_export_matches_lumen_format_byte_for_byte() {
    ...
    let spec = line_comment::path_to_uri(&root.join("docs").join("spec.md"));
    let plan = line_comment::path_to_uri(&root.join("docs").join("plan.md"));
...
fn copy_writes_the_export_and_names_the_path() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            Effect::ShowMessage {
                error: false,
                text: "1 comment → .tmp/line-comment.md".to_string()
            }
        ]
    );

    session.write_export().unwrap();
    let written = std::fs::read_to_string(root.join(".tmp").join("line-comment.md")).unwrap();
...
fn an_unsupported_store_version_starts_empty_and_reports_it() {
    ...
    std::fs::create_dir_all(root.join(".tmp")).unwrap();
    std::fs::write(
        root.join(".tmp").join("line-comment.json"),
        r#"{"version":99,"files":{}}"#,
    )
    .unwrap();
...
fn every_file_type_is_served() {
    ...
    for (path, key) in [
        ...
        assert_eq!(
            session.key(&line_comment::path_to_uri(&root.join(path))),
            Some(key.to_string()),
...
fn the_servers_own_scratch_files_are_not_served() {
    ...
    for name in [
        "line-comment.md",
        "line-comment.json",
        "line-comment-list.md",
        "line-comment-input-1755769123456.md",
    ] {
        assert!(
            session
                .key(&line_comment::path_to_uri(&root.join(".tmp").join(name)))
                .is_none(),
...
fn a_file_outside_the_root_keeps_its_absolute_path() {
    ...
    assert_eq!(
        session.key(&line_comment::path_to_uri(&outside)),
...
fn list_writes_the_export_and_hands_over_a_view() {
    ...
    assert!(session.is_scratch_path(line_comment::scratch::LIST, &path));
    // The export keeps its fixed path — that is the one the Claude command reads.
    assert_eq!(
        session.export_path(),
        root.join(".tmp").join("line-comment.md")
    );
}
