...
1. Read `insights_root` from `~/.claude/memory-keeper.local.md` YAML frontmatter. If the file is missing, stop and ask the user to create it with the required settings.
2. If no query -> call `memory_context` MCP tool to get project overview
...
1. Call `memory_context` MCP tool for project overview
2. Call `memory_topics` MCP tool for detailed topic listing
