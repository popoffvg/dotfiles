...
Step 0b names the source and nothing else. A `correction` or a `method` always continues to Step 1, which decides scope on its own terms — Step 0b never argues for `skip`. Only `decision` stops here.
...
1. **A stated method is a lesson.** Do not wait for a correction. If the user said how they want a task done — or how this repo works — run the same steps on it (Step 0b).
