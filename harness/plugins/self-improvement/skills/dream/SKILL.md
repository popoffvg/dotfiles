...
The Stop hook's triage subagent ([[capture-lesson]] `references/triage.md`) only classifies a session — global behaviour correction, project-scope convention, or neither — and archives the first two under `~/.claude/self-improvement/lessons/<scope>/<date>-<topic-slug>-<session-id>.jsonl`. No recurrence judgment happens there: the subdir is a cheap haiku guess at scope, not a verdict, and nobody has yet judged whether the lesson is real. `dream` closes that loop, in batch, before consolidating:

1. **List unharvested archives.** `~/.claude/self-improvement/lessons/global/*.jsonl` and `~/.claude/self-improvement/lessons/project/*.jsonl`. Harvested transcripts live in `~/.claude/self-improvement/lessons/harvested/` and are done. Take the subdir as triage's guess and re-judge scope yourself in step 4 — a `global/` transcript can still turn out to be project-scoped.
...
4. **Run capture-lesson on what survives.** For each correction that recurs, run [[capture-lesson]] Steps 1–5 (scope, form, find-or-write skill, `CASE.md`, report) as if the Stop hook had run it live. Feed the sidecar's repo rows into Step 1's scope choice and into the `CASE.md` `Repo:` field. Then move the `.jsonl` and its `.env.md` into `~/.claude/self-improvement/lessons/harvested/` and record that final path in the `CASE.md` `Transcript:` field, so the evidence stays reachable and the transcript is never harvested twice.
