#!/usr/bin/env python3
"""Drive line-comment-lsp over stdio the way Zed does, and print what an editor would see.

Proves the server side of the line-comment extension in isolation: capabilities,
...
usage: line-comment-probe.py [<markdown-file>] [--binary <path>] [--line N]
...
def main() -> int:
    args = sys.argv[1:]
    binary = os.path.expanduser("~/.local/bin/line-comment-lsp")
    if "--binary" in args:
    ...
    if args:
        ...
        temporary = tempfile.mkdtemp(prefix="line-comment-probe-")
        root = Path(temporary)
    ...
    try:
        ...
        add = next(
            (a for a in actions or [] if a["command"]["command"] == "line-comment.add"), None
        )
        if add is None:
        ...
        store = root / ".tmp" / "line-comment.json"
