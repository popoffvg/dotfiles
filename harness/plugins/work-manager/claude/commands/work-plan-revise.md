---
name: work-plan-revise
description: Revise plan.md and todos/TODO-N.md so they match what the last commit related to TODO-N actually implemented.
argument-hint: <TODO-N> [<sha-or-range>]
---

Use the `plan-revise` skill to update the plan to match reality.
...
2. Follow the `plan-revise` skill end-to-end:
   - Locate the commit(s) for the given TODO.
   - Inspect the diff (`git show <sha>`) and categorize deltas vs the current plan.
   - Update `<notes-dir>/plan.md` and `<notes-dir>/todos/TODO-N.md` so they reflect what actually shipped.
