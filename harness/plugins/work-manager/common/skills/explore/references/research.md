# Explore › research — document codebase as-is

`/explore research` — `model: opus`.

Conduct comprehensive research across the codebase to answer a question by spawning
parallel sub-agents and synthesizing their findings into a dated write-up under
`<notes-dir>/research/`.
...
If invoked without a research query, respond with:
```
...
4. **Wait for all sub-agents to complete and synthesize findings:**
   ...
   - Verify all thoughts/ paths are correct (e.g., thoughts/allison/ not .notes/ for personal files)
   - Highlight patterns, connections, and architectural decisions
   - Answer the user's specific questions with concrete evidence
   - **Build the glossary**: collect every domain term each researcher surfaced (from their **Terms** list), dedupe, and record relationships between them (e.g. "X contains Y", "A triggers B", "P is a kind of Q"). This feeds the `## Glossary` section.

5. **Gather metadata for the research document:**
   - Run the `hack/spec_metadata.sh` script to generate all relevant metadata
   - Filename: `.notes/research/YYYY-MM-DD-ENG-XXXX-description.md`
...
6. **Generate research document:**
   ...
   - Structure the document with YAML frontmatter followed by content:
     ...
     **Repository**: [Repository name]
     ...
     ## Glossary
     Table of domain terms surfaced by all researchers, plus relationships between them.

     | Term | Definition | Related terms |
     |---|---|---|
     | [term] | [what it means in this codebase] | [related term — relationship, e.g. "contains Y", "triggers Z"] |
...
- **Terms** — domain terms you encountered: `term — short definition` + any relationship to other terms (e.g. "contains", "triggers", "is a kind of"). Feeds the synthesizer's glossary. Omit if none.
...
- **Path handling**: The thoughts/searchable/ directory contains hard links for searching
  ...
  - Examples of correct transformations:
    ...
    - `thoughts/searchable/shared/prs/123.md` → `.notes/prs/123.md`
