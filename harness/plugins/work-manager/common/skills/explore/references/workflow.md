...
`$RESEARCH_DIR/flows.json` that `explore flow-map` renders as interactive HTML. Schema:
...
8. **Print** the research dir path and suggest `explore flow-map` against `$RESEARCH_DIR/flows.json` for an interactive HTML view.
