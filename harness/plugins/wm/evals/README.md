...
- **`half`** → `human` | `agent` : which half of the pair the content belongs in. `human` is
  `TODO-N.md` (Outcome, New terms, Components, **Surface**, Autotest, Commit); `agent` is
  `TODO-N.agent.md` (Constraints, Changes, Files, Pre-reads, Manual test, Definition of done).
  **Every diff is human** — `## Surface` is the one diff in the pair, and the agent half carries none.
...
The runner extracts four rule blocks verbatim from the skill at run time (§ One ledger row, two
files; § Surface, which nests the surface-not-a-body rule; § Changes; § Autotest), so the eval always
grades the current spec. Change a rule → re-run;
...
26 cases, all four quadrants: 12 `human`/`keep`, 6 `agent`/`keep`, 7 `human`/`reshape`,
1 `agent`/`reshape`. The distribution leans human because the diff lives there now.
...
- `constraints-table` — the one section whose half is counter-intuitive. It reads like design, so it
  ...
  increments in the agent half.

## Last run

2026-08-21, `MODEL=sonnet`, two axes, 26 cases, after the diff moved to `## Surface` in the human
