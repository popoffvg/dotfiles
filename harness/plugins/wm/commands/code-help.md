...
| `auto` | Run the whole ledger unattended: arm the `/goal` Stop hook, then loop every open TODO through read `LESSONS.md` → implement → `reviewer` gate → `tester` gate (writes the missing test) → append `LESSONS.md`, then deploy, then verify the last TODO's E2E command. No per-increment approval. |
