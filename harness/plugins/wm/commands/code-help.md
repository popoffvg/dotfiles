...
| Subcommand | Does |
|---|---|
| `new` *(default)* | Spec pipeline: write spec.md (if missing) → grill until no open question note is left in thoughts/ → compile the plan with its wave table → stop at the gate. Does not write TODO bodies — the human reviews the spec first. |
