...
- **code**: one router — `/code <new|verify|revise|quiz|prototype|code-map|impl|tree|squash|fix|help>` (write spec, grill until no open question note is left, auto-write TODO bodies, audit, sync, prototype, diagram, implement one TODO, implement-in-`wt`-worktree + merge, distill worktree fixup lessons, fix bugs)
- **impl-verify**: adversarial post-impl verification (the one surviving `impl-` skill). Commit conventions moved to `/code commit`; the worktree flow is now the spec's `where: worktree` key, which makes `/code impl` work in the branch's own `wt` worktree, with `/code squash` for fixup learning and the merge back.
- **test**: one router — `/test-suite <create|write|verify|case-design|bdd|tdd|harness|review>` (design a pairwise-tiered strategy, enumerate the scenarios, audit an existing set for gaps and readability, derive cases by black-box technique, shape BDD scenarios, drive spec-before-code TDD, test a plugin harness, run the verify-phase review)
