...
| `impl` | Execute one TODO — read context, replan guard, apply the increments and show them for approval as the `approve` key asks, autotest, commit, report. `implementer` (sonnet, bg) writes + commits; the gate chain is `review`. |
