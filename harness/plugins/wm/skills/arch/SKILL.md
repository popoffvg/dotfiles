...
description: >
  ...
  thoughts/, the todos/ TODO-N.md + TODO-N.agent.md pair and its TODO-N.trace.md companion, GLOSSARY.md), and the brick roster: the closed set of component types
...
| `todo` | Author the self-contained `todos/TODO-N.md` + `TODO-N.agent.md` pair, and the `TODO-N.trace.md` companion recording where each decision in it came from, from a reviewed `spec.md` + `thoughts/`. Runs only past the gate. | `commands/sub-todo.md` |
...
artifact — three for a TODO: `tpl-todo.md` for the human half, `tpl-todo-agent.md` for the agent
half, and `tpl-todo-trace.md` for the trace. What each owns is in `wm:INDEX.md`; this skill does not restate it.
