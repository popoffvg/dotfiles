...
Always call @implementer with background: true for implementation. **Name the checkout path in every delegation** — the one step 3 resolved. A background agent starts in the session cwd, so a `where: worktree` TODO whose prompt carries no path is implemented in the wrong tree.
