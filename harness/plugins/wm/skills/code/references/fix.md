...
6. Commit as a **fixup** into the commit the fix belongs to (see below).

### Committing the fix

A fix amends work already done — fold it into the commit it corrects rather than stacking a new one.

1. Stage only the files changed: `git add <files>`.
2. Find the fixup target among unpushed local commits:
   ```
   git log --oneline '@{upstream}..HEAD' 2>/dev/null || echo NO_UPSTREAM
   ```
   - Commits listed → pick the one whose message matches the TODO / thought being fixed (usually the commit that implemented it).
   - `NO_UPSTREAM` → all commits are local; pick the most relevant.
   - Empty output → all commits pushed; do NOT rewrite them, create a new commit instead (step 4).
3. **Fixup target found** — create the fixup commit only:
   ```
   git commit --fixup=<target-hash>
   ```
   Leave it unsquashed. Collapsing fixups into their targets is `/code squash`, run manually by the user — never autosquash here.
4. **No unpushed target** — new commit referencing both notes:
   ```
