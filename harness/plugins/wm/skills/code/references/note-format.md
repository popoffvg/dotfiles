...
- `status` — `approved` (default) or `declined`. A note is `approved` on write; set `declined` when the user rejects the thought (superseded or wrong) instead of deleting it — the reasoning stays in the graph.
