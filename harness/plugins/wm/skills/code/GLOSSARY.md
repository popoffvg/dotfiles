...
| **constraints** | A TODO's `## Constraints` table — every settled decision it obeys, restated in full, one row per `thoughts` entry. It exists so the TODO is self-contained: the implementer never opens `spec.md` or a note. |
