...
Agent(subagent_type="wm:spec-verifier", model="sonnet", prompt=
  "[VERIFY TODO-N] Hunt contradictions / missing-parts / edge-cases in the <notes-dir>/todos/TODO-N.md
   + TODO-N.agent.md pair and its TODO-N.trace.md. Read only those three + the pair's Files (source),
   plus any thoughts/ note the trace cites. Follow ${CLAUDE_PLUGIN_ROOT}/skills/code/commands/sub-verify.md § Mission.
...
Three files per ledger row, contiguous: `todos/TODO-N.md` (the human half), `todos/TODO-N.agent.md` (the agent half) and `todos/TODO-N.trace.md` (the trace). A row missing any of them is a finding — the human half alone is unimplementable, the agent half alone is work nobody approved, and a pair with no trace is work whose reasons cannot be recovered. Each file has every `always` element in order (`arch:sub-todo.md` § Required elements), and **nothing belonging to another**: a `## Changes` or `## Constraints` in the human half, a `## Surface` or `## Commit` in the agent half, a `## Trace` outside the trace, or **any ```diff outside the human half**, means the split was never made.

Spot-check the human half: **risk** 1–5 with a justification (score ≥ 3 → tests cover callers); `## Components` ≤ 5 rows, exactly one `main`; `## Surface` covers every Components symbol and no others, one ```diff per file, ≤ 150 changed lines each or a declared **Compile floor**. Spot-check the agent half: **Files** concrete paths, no globs; every `## Constraints` row carries a `C<n>` id and the rule alone, and every settled decision an increment can violate has a row.

**The trace.** `## Trace` is its only section, and every row fills `Anchor`, `Origin` and `Why here`. Each **Anchor** names something that exists in the pair — an anchor pointing at a deleted symbol, a renumbered `C<n>`, or an increment that no longer exists is a stale row and a finding. Each **Origin** resolves: a `[[NNN-type-slug]]` to a **live** note in `thoughts/`, or a document with a section and a `read <YYYY-MM-DD>` date. **A note that resolves only under `thoughts/archived/` is a hard block** — the decision was superseded while this TODO still obeys it, so the pair itself may be wrong; route to `revise`. Check the floor: an `Outcome` row, one row per `C<n>`, one row per impl-decision note written for this TODO, and one per external document a `## Surface` shape or setting value came from. `python3 <plugin>/bin/budget-check.py <trace file>` reports the mechanical half of this; the anchors-still-exist judgment is the audit's.
