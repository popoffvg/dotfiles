...
**Changes — the increment sequence.** `n` contiguous from 1, ≤ 10 increments, each naming one **Components** row. Every increment carries **Files** (a subset of `## Files`), a **Blast radius**, and a ```diff of ≤ 150 changed lines — over that only with a **Compile floor** bullet and at least one `AGENT: implement in increment <n>` stub, per `arch:sub-todo.md` § Changes. A **Blast radius** that names no symbol or caller (`"low"`, `"minimal"`, `"none"` on a non-additive increment) → NEEDS REVISION: an unpredicted blast radius is what the increment review exists to catch. Order must be deepest-first — a caller migrated before its callee, without a `builds: only with increment <n>` marker, is a finding. Any **Behavior** snippet: one TS block ≤ 40 lines matching the Type.
