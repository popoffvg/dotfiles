...
4. **Glossary** — if the change introduces or renames a domain term, update `<notes-dir>/GLOSSARY.md` in the same commit.
5. **Autotest** — run the TODO's `## Autotest`. Must be green before committing.
6. **Commit** — on green, commit per `sub-commit.md`. The spec's `## Commit` block is the primary message.
