...
> **Map**: @../../INDEX.md — which of the five skills owns which file, and what that file owns. This router restates none of it.
...
> **Notes history**: @references/ref-jj-notes.md — the notes-dir is a symlink to its own jj repo under `~/.notes`; declaring one is `bin/note-declare.sh`, history is `jj log`.
...
|---|---|---|
| `new` *(default)* | Spec pipeline: init the corpus (`CLAUDE.md`, `RULES.md` — ask the three rule questions plus the spec's `approve` depth — `CONSTRAINTS.md`, `PATTERNS.md`), write `spec.md` (if missing) → grill until no open question note is left in `thoughts/` → compile the plan with its **wave** table → **stop at the gate**. Does not write TODO bodies. | `arch:sub-new.md` · `skill:flow-scetch` |
| `todo` | Author the self-contained `todos/TODO-N.md` + `TODO-N.agent.md` pair — human half (Outcome, Components, **Surface** — the one diff — Unit **and** E2E tests) and agent half (the `CONSTRAINTS.md` pointer + the increments described, no diff, unlimited length) — appending every settled decision it finds to `CONSTRAINTS.md`. From a reviewed `spec.md` + `thoughts/`; runs only past the gate. | `arch:sub-todo.md` · `skill:flow-scetch` |
...
| `impl` | Execute one TODO — read context, replan guard, apply the increments and show them for approval as the spec's `approve` key asks, autotest, commit, report. | `impl:sub-impl.md` |
