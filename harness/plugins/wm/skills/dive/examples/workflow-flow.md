...
> Layout rules: `sub-workflow.md` § Procedure. Shared-file rules:
> `sub-workflow.md` § Verify + autocomplete plumbing.
> The four shared files sit at `$WORKFLOWS_DIR` itself:
...
> `.workflow.ts` rules: `sub-workflow.md` § Rules — the `.workflow.ts`.
...
> A component declaration exposes the pseudocode API and binds its symbols with `@source`; citation
> verification is `sub-workflow.md` § Verify + autocomplete plumbing.
...
> A sibling bindings file maps each notable-branch ULID to its verified source; see
> `sub-workflow.md` § Verify + autocomplete plumbing.
...
> The shared config covers every flow folder; see `sub-workflow.md` § Procedure.
...
> `flows.json` contract and aggregation rules: `sub-workflow.md` § Rules — `flows.json`.
