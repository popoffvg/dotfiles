...
|---|---|---|
| `zed-pr-worktree.sh` | Github: PR review | Pick a PR (fzf over `gh pr list`), reuse its worktree or create one at `<repo>.pr-<N>`, set `git.diff_base: "default_branch"` in the worktree's `.zed/settings.json`, open it with `zed --existing`. |
